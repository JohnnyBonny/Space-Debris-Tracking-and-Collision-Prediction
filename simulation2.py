...
class simulation2:
  def __init__(self,satellites:list[satellite2.satellite2],tolerance_zone:int,collision_zone:int,start_date:datetime,end_date:datetime,increments:list[int],playback_speed:int,repeat:bool):
    ...
    if validators.url(url):
      ...
      if response.status_code == 200:
        ...
        lines = response.text.strip().splitlines()
          ...
          print("populate satellite function does not work")
          return
  ...
  def point_calculations(self, date ,points:list):
      ...
      if self.validate_sim_date(self.start_date,self.end_date,self.increments) == True:
        ...
        while future_time < self.end_date:
          ...
          for satellite in self.satellites:
            ...
            #this assumes we are getting the coordinates from a source that has 3 lines
            satellite.get_coordinates(jd,fr)

            points.append(np.array([satellite.get_x_position(), satellite.get_y_position(), satellite.get_z_position()]))

          #do the calculations here
          self.point_calculations(future_time,points)
